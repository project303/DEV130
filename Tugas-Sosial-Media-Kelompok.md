# Tugas Sosial Media Kelompok

1. Buat 3 postingan dengan 3 topik berbeda pada Instagram dengan topik berikut:
   - Data Warehouse
   - Medallion Architecture
   - Data Lakehouse
2. Postingan dapat berupa Reels atau Carousel (min. 6 slide)
3. Postingan dibuat perkelompok dan **diposting pada IG masing-masing**
4. Caption berisi informasi:
   - topik yang dibahas
   - list nama kelompok
   - hash tag: #datalearns247 #belajarbigdata #ngulikbigdata #bigdataindonesia
   - mentag akun @datalearns247 **(bukan kolaborasi)**
5. Contoh postingan
   - https://www.instagram.com/p/Cf_aQ6GLHJY/?igshid=NjZiM2M3MzIxNA==
6. Postingan paling lambat dipublish **3 Januari 2026** (sesuai pada tanggal postingan pada IG)
7. Melengkapi form berikut sebagai bukti telah memposting:
   [Form Sosial Media](https://docs.google.com/forms/d/1iZ9T59BOxkLNd65tkA8MiGysd_IOfl9Dl_IMRTqAd4s/viewform)
